...
--!!!!!!!!!!!!!!!!!!!!!!!!!!!
--Find the context menu function for the "right-click" context menu at normal inventory items in filename
--"FCOIS_Hooks.lua", function name "FCOItemSaver_AddSlotAction"
--!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
function FCOIS.hideContextMenu(whichContextMenu)
    ...
    if whichContextMenu == nil or whichContextMenu == -1 then
        ...
        --Hide the lock & dynamic split filter button context-menu
        ...
        --Hide the SELL & SELL IN GUILD STORE & INTRICATE button context-menu
...
function FCOIS.refreshPopupDialogButtons(rowControl, override)
    ...
    ClearCursor()
    --Button 1 must not ne disabled if we are not inside the research popup (but inside the wayshrine port popup, or weapon enchant popup, or ...)
    --This got probably to do with the "SetKeyboardEnabled(true) function!
    --is the research dialog shown?
    if not ZO_ListDialog1:IsHidden() then
        local ctrlVars = FCOIS.ZOControlVars
        local disableResearchNow = false
        if not override then
            local isRepairItemDialog = ZO_Dialogs_IsShowing(ctrlVars.RepairItemDialogName)

            --get the marked icons of the item
            local data = rowControl.dataEntry.data
            local _, markedIcons = FCOIS.IsMarked(data.bag, data.index, -1)
            if markedIcons then
...
--d("[FCOIS.refreshPopupDialogButtons]row: " ..tostring(rowControl:GetName()) .. ", disable: " ..tostring(disableResearchNow))
            ClearCursor()
...
function FCOIS.AddMark(rowControl, markId, isEquipmentSlot, refreshPopupDialog, useSubMenu)
    ...
    if not useSubMenu and settings.addContextMenuLeadingSpaces > 0 then
        ...
        for indentNo = 1, settings.addContextMenuLeadingSpaces do
    ...
    if not useSubMenu and firstAdd then
        ...
        if settings.showContextMenuDivider then
            ...
            elseif settings.contextMenuDividerClearsMarkers
                    and not isEquipmentSlot and FCOIS.ZOControlVars.LIST_DIALOG:IsHidden() then
        ...
        if firstAdd and settings.showContextMenuDivider then
            if contMenuVars.contextMenuIndex ~= -1 and ZO_Menu.items[contMenuVars.contextMenuIndex] ~= nil then
                ...
                if contextMenuItemControl~= nil then
                    if ( (isEquipmentSlot) or  (not FCOIS.ZOControlVars.LIST_DIALOG:IsHidden())
        ...
        if doAbort == false then
            ...
            if (updateNow == true) then
                ...
                if FCOIS.IIfAclicked ~= nil then
                    ...
                    if rowControl:GetParent() == FCOIS.ZOControlVars.CHARACTER then
                        FCOIS.RefreshEquipmentControl(rowControl, not doUnmark, markId)
                    elseif rowControl:GetParent():GetParent() == FCOIS.ZOControlVars.QUICKSLOT_LIST then
                        FCOIS.FilterBasics(false)
                    else
                        FCOIS.FilterBasics(false)
                    end
...
--d(">>> Sort order " .. settings.icon[i].sortOrder .. " is duplicate!")
    ...
    if not returnDuplicates then
        ...
        for index, rescanData in ipairs(rescanDuplicatesTable) do
...
local function getFilterButonContextMenuActiveIcons(contextMenuType)
    ...
    local isIconEnabled = FCOIS.settingsVars.settings.isIconEnabled
    for index, buttonNameStr in ipairs(filterContextMenuButtonTemplateIndex) do
...
--d("[FCOIS] rebuildFilterButtonContextMenuVars")
    ...
    ctmName = ""
    local ctmName = tostring(availableCtms[FCOIS_CON_FILTER_BUTTON_GEARSETS])
...
--d("[FCOIS.showContextMenuFilterButton] Parent name: " .. parentButton:GetName() .. ", Type: " .. contextMenuType .. ", PanelId: " .. FCOIS.gFilterWhere)
    ...
    for index, buttonNameStr in ipairs(filterButtonContextMenuButtonTemplateIndex) do
...
-- ========================================================================================================================
-- ============================================================
--         Additional inventory button context menu
-- ============================================================
--Function to build the text for the toggle buttons (Anti-Deconstruct, Anti-Destroy, Anti_Sell, etc. FCOIS.settingsVars.settings)
...
function FCOIS.getContextMenuAntiSettingsTextAndState(p_filterWhere, buildText)
    ...
    --Get the current anti-settings state
    if   p_filterWhere == LF_INVENTORY or p_filterWhere == LF_BANK_WITHDRAW or p_filterWhere == LF_GUILDBANK_WITHDRAW or p_filterWhere == LF_HOUSE_BANK_WITHDRAW
      or p_filterWhere == LF_BANK_DEPOSIT or p_filterWhere == LF_GUILDBANK_DEPOSIT or p_filterWhere == LF_HOUSE_BANK_DEPOSIT then
    ...
    elseif p_filterWhere == LF_CRAFTBAG then
        ...
        if FCOIS.checkIfCBEorAGSActive(FCOIS.gFilterWhereParent) then
            ...
            --The parent panel for the craftbag is the guild bank deposit panel
            if 		parentPanel == LF_GUILDBANK_DEPOSIT then
                currentSettingsState = settings.blockDestroying

            --The parent panel for the craftbag is the bank deposit panel
            elseif 		parentPanel == LF_BANK_DEPOSIT then
                currentSettingsState = settings.blockDestroying

            --The parent panel for the craftbag is the house bank deposit panel
            elseif 		parentPanel == LF_HOUSE_BANK_DEPOSIT then
                currentSettingsState = settings.blockDestroying

            --The parent panel for the craftbag is the mail send panel
            elseif	parentPanel == LF_MAIL_SEND then
                currentSettingsState = settings.blockSendingByMail

            --The parent panel for the craftbag is the guild store sell panel
            elseif	parentPanel == LF_GUILDSTORE_SELL then
                currentSettingsState = settings.blockSellingGuildStore

            --The parent panel for the craftbag is the trade panel
            elseif	parentPanel == LF_TRADE then
                currentSettingsState = settings.blockTrading
            end
        else
            currentSettingsState = settings.blockDestroying
        end
    ...
    elseif p_filterWhere == LF_FENCE_LAUNDER then
        currentSettingsState = settings.blockLaunder
    elseif p_filterWhere == LF_SMITHING_REFINE then
        currentSettingsState = settings.blockRefinement
    elseif p_filterWhere == LF_SMITHING_DECONSTRUCT then
        currentSettingsState = settings.blockDeconstruction
    elseif p_filterWhere == LF_SMITHING_IMPROVEMENT then
        currentSettingsState = settings.blockImprovement
    elseif p_filterWhere == LF_SMITHING_RESEARCH then
        currentSettingsState = false -- Research tab got no additional filter button or settings so always simulate it as OFF (false)
    elseif p_filterWhere == LF_JEWELRY_REFINE then
        currentSettingsState = settings.blockJewelryRefinement
    elseif p_filterWhere == LF_JEWELRY_DECONSTRUCT then
        currentSettingsState = settings.blockJewelryDeconstruction
    elseif p_filterWhere == LF_JEWELRY_IMPROVEMENT then
        currentSettingsState = settings.blockJewelryImprovement
    ...
    elseif p_filterWhere == LF_ENCHANTING_CREATION then
        currentSettingsState = settings.blockEnchantingCreation
    elseif p_filterWhere == LF_ENCHANTING_EXTRACTION then
        currentSettingsState = settings.blockEnchantingExtraction
    elseif p_filterWhere == LF_RETRAIT then
        currentSettingsState = settings.blockRetrait
    else
        currentSettingsState = settings.blockDestroying
    end
...
--The context menu OnClicked callback function for the additional inventory flag context menu buttons
...
--d("[FCOIS]ContextMenuForAddInvButtonsOnClicked - buttonCtrl: " .. tostring(buttonCtrl:GetName())  .. ", iconId: " .. tostring(iconId)  .. ", doMark: " .. tostring(doMark)  .. ", specialButtonType: " .. tostring(specialButtonType))
    ...
    --House Banks
        ...
        if FCOIS.checkIfCBEorAGSActive(FCOIS.gFilterWhereParent, true) and INVENTORY_CRAFT_BAG and not ctrlVars.CRAFTBAG:IsHidden() then
            ...
            inventoryType = mappingVars.InvToInventoryType[FCOIS.gFilterWhere]
    ...
    if not isUNDOButton and not isREMOVEALLGEARSButton and not isREMOVEALLButton and not isTOGGLEANTISETTINGSButton then
                            ...
                            if myItemInstanceId ~= nil then
                                ...
                                if doMark == true then
                                    ...
                                    if not FCOIS.checkIfItemIsProtected(iconId, myItemInstanceId) then
                                        ...
                                        --Check if the sell/sell at guild store icon should be demarked if this icon get's set
                                        ...
                                        --FCOIS.markedItems[iconId][FCOIS.SignItemId(myItemInstanceId)] = true
                                ...
                                elseif doMark == false then
                                    ...
                                    if FCOIS.checkIfItemIsProtected(iconId, myItemInstanceId) then
                                        --FCOIS.markedItems[iconId][FCOIS.SignItemId(myItemInstanceId)] = nil
                ...
                if v ~= nil and data ~= nil then
                    ...
                    if bagId ~= nil and slotIndex ~= nil then
                        ...
                        if myItemInstanceId ~= nil then
                            ...
                            for iconId, _ in pairs(mappingVars.iconToGear) do
                                ...
                                if allowedToMark then
                                    ...
                                    if FCOIS.checkIfItemIsProtected(iconId, myItemInstanceId) then
                                        ...
                                        --d("[REMOVE ALL GEARS] ADD slotIndex: " .. tostring(slotIndex) .. ", bag: " .. tostring(bagId) .. ", iconId: " .. tostring(iconId) .. ", marked: false")
                        ...
                        if myItemInstanceId ~= nil then
                            ...
                            for iconId = 1, numFilterIcons, 1 do
                                ...
                                if FCOIS.checkIfItemIsProtected(iconId, myItemInstanceId) then
                                    ...
                                    --d("[REMOVE ALL] ADD slotIndex: " .. tostring(slotIndex) .. ", bag: " .. tostring(bagId) .. ", iconId: " .. tostring(iconId) .. ", marked: false")
                            ...
                            end -- for iconId = 1, numFilterIcons, 1 do
                        end
...
--Function that is called upon onMouseUp event on the additional inventory context menu buttons
...
function FCOIS.showContextMenuForAddInvButtons(invAddContextMenuInvokerButton)
    ...
    --Hide the other filter button contextmenus first
    ...
    local antiButtonText, settingsEnabled = FCOIS.getContextMenuAntiSettingsTextAndState(panelId, true)
