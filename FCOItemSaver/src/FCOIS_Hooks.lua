...
local function FCOItemSaver_OnReceiveDrag(inventorySlot)
    ...
    local cursorContentType = GetCursorContentType()
    if FCOIS.settingsVars.settings.debug then debugMessage( "[OnReceiveDrag]","cursorContentType: " .. tostring(cursorContentType) .. "/" .. tostring(MOUSE_CONTENT_INVENTORY_ITEM) .. ", invSlotType: " .. tostring(inventorySlot.slotType) .. "/" .. tostring(SLOT_TYPE_EQUIPMENT), true, FCOIS_DEBUG_DEPTH_NORMAL) end
...
function FCOIS.CreateHooks()
    ...
    ZO_PreHook("EquipItem", function(bagId, slotIndex, equipSlotIndex)
        ...
        if bagId ~= nil and slotIndex ~= nil then
            if settings.debug then debugMessage( "[EquipItem]","bagId: " .. bagId .. ", slotIndex: " .. slotIndex .. ", equipSlotIndex: " ..tostring(equipSlotIndex), true, FCOIS_DEBUG_DEPTH_NORMAL) end
    ...
    SecurePostHook("RequestEquipItem", function(bagId, slotIndex, bagWorn, equipSlot)
        ...
        if settings.debug then debugMessage( "[RequestEquipItem]","bagId: " ..tostring(bagId) .. ", slotIndex: " ..tostring(slotIndex) .. ", bagWorn: " ..tostring(bagWorn) .. ", equipSlotIndex: " .. tostring(equipSlot), true, FCOIS_DEBUG_DEPTH_NORMAL) end
    ...
    SecurePostHook("RequestUnequipItem", function(bagId, equipSlot)
        if bagId ~= nil and equipSlot ~= nil then
            if settings.debug then debugMessage( "[RequestUnequipItem]","bagId: " ..tostring(bagId) .. ", equipSlotIndex: " .. equipSlot, true, FCOIS_DEBUG_DEPTH_NORMAL) end
    ...
    if enchantPopupDialogCustomControl ~= nil then
        ZO_PostHookHandler(enchantPopupDialogCustomControl, "OnHide", function()
            FCOIS.CheckIfEnchantingInventoryItemShouldBeReMarked_AfterEnchanting()
        end)
    end
    ...
    ctrlVars.LIST_DIALOG.dataTypes[1].setupCallback = function(rowControl, slot)
        ...
        ZO_PreHookHandler(rowControl, "OnMouseUp", function(control, button, upInside, ctrlKey, altKey, shiftKey, command)
            if settings.debug then debugMessage( "[Research dialog control-OnMouseUp]","Clicked: " ..control:GetName() .. ", MouseButton: " .. tostring(button), true, FCOIS_DEBUG_DEPTH_NORMAL) end
    ...
    LOOT_SCENE:RegisterCallback("StateChange", function(oldState, newState)
        if settings.debug then debugMessage( "[LOOT_SCENE]","State: " .. tostring(newState), true, FCOIS_DEBUG_DEPTH_NORMAL) end
    ...
    local function smithingSetModeHook(smithingCtrl, mode, ...)
        ...
        if settings.debug then debugMessage( "[SMITHING:SetMode]","Mode: " .. tostring(mode), true, FCOIS_DEBUG_DEPTH_NORMAL) end
--d("[FCOIS]smithingSetModeHook-mode: " ..tostring(mode) .. ", filterPanel: " ..tostring(FCOIS.gFilterWhere))
    ...
    local function enchantingPreHook()
        ...
        zo_callLater(function()
            ...
            if enchantingCtrl:IsSceneShowing() then
                ...
                if settings.debug then debugMessage( "[ENCHANTING:OnModeUpdated]","EnchantingMode: " .. tostring(enchantingMode), true, FCOIS_DEBUG_DEPTH_NORMAL) end
    ...
    ctrlVars.CRAFT_BAG_FRAGMENT:RegisterCallback("StateChange", function(oldState, newState)
        ...
        if 	newState == SCENE_FRAGMENT_SHOWING then
            ...
            if settings.debug then debugMessage( "[CRAFT_BAG_FRAGMENT]", ">Parent panel: " .. tostring(parentPanel), true, FCOIS_DEBUG_DEPTH_VERY_DETAILED) end
            ...
            if cbeOrAGSActive and parentPanel ~= nil then
                ...
                if supportedPanels[parentPanel] then
                    ...
                    if settings.debug then debugMessage( "[CRAFT_BAG_FRAGMENT]", ">supported craftbag parent panel: " .. tostring(FCOIS.gFilterWhereParent), true, FCOIS_DEBUG_DEPTH_VERY_DETAILED) end
                end
            end
        ...
        elseif  newState == SCENE_FRAGMENT_HIDDEN then
            ...
            if settings.debug then debugMessage( "[CRAFT_BAG_FRAGMENT]", ">new panel: " .. tostring(FCOIS.gFilterWhere), true, FCOIS_DEBUG_DEPTH_VERY_DETAILED) end
    ...
    ctrlVars.MAIL_SEND_SCENE:RegisterCallback("StateChange", function(oldState, newState)
        if settings.debug then debugMessage( "[MAIL_SEND_SCENE]","State: " .. tostring(newState), true, FCOIS_DEBUG_DEPTH_NORMAL) end
        ...
        for _, sceneControl in ipairs(sceneControlsToRegisterStateChangeForContextMenu) do
            if sceneControl ~= nil and sceneControl.RegisterCallback then
                sceneControl:RegisterCallback("StateChange", function(oldState, newState)
                    ...
                    if settings.debug then debugMessage( "[" .. tostring(sceneName) .. "]","State: " .. tostring(newState), true, FCOIS_DEBUG_DEPTH_NORMAL) end
                    sceneCallbackHideContextMenu(oldState, newState)
    ...
    ctrlVars.RETRAIT_KEYBOARD_INTERACT_SCENE:RegisterCallback("StateChange", function(oldState, newState)
        if settings.debug then debugMessage( "[RETRAIT SCENE]","State: " .. tostring(newState), true, FCOIS_DEBUG_DEPTH_NORMAL) end
        sceneCallbackHideContextMenu(oldState, newState)
        if     newState == SCENE_SHOWING then
            --Check if craftbag is active and change filter panel and parent panel accordingly
    ...
    ctrlVars.COMPANION_INV_FRAGMENT:RegisterCallback("StateChange", function(oldState, newState)
        if settings.debug then debugMessage( "[COMPANION EQUIPMENT FRAGMENT]","State: " .. tostring(newState), true, FCOIS_DEBUG_DEPTH_NORMAL) end
    ...
    ctrlVars.COMPANION_CHARACTER_FRAGMENT:RegisterCallback("StateChange", function(oldState, newState)
        if settings.debug then debugMessage( "[COMPANION CHARACTER FRAGMENT]","State: " .. tostring(newState), true, FCOIS_DEBUG_DEPTH_NORMAL) end
